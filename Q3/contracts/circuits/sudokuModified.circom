...
template sudoku() {
    ...
    component range1[9][9];
    component range2[9][9];
    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
            range1[i][j] = RangeProof(8);
            range1[i][j].range[0] <== 0;
            range1[i][j].range[1] <== 9;

            range2[i][j] = RangeProof(8);
            range2[i][j].range[0] <== 0;
            range2[i][j].range[1] <== 9;
        }
    }

    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
            range1[i][j].in <== puzzle[i][j];
            assert(range1[i][j].out==1);

            range2[i][j].in <== solution[i][j];
            assert(range2[i][j].out==1);
